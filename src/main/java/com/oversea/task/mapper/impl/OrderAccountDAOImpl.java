...
public class OrderAccountDAOImpl extends BaseDao implements OrderAccountDAO {
	...
	@Override
	public List<OrderAccount> getNeedRechargeAccountByStatusByGroup(
			String status, String expressCompanyIds, String siteNames) {
		Map<String, Object> map = new HashMap<String, Object>();
        map.put("expressCompanyIds", expressCompanyIds);
        map.put("status", status);
        map.put("siteNames", siteNames);
	...
	@Override
	public int updateOrderFirstNameAndRecipients(OrderAccount orderAccount) {
        return getSqlSession().update("OrderAccountMapper.updateOrderFirstNameAndRecipients", orderAccount);
	}
}
